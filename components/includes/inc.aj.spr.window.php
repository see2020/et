<?php
/**
 * inc.aj.spr.window.php - ќкно выбора из таблиц с типом справочник
 *
 * "/aj.php?af=spr.window".
 * "&selected_val=".spr_JsUrlEscape($qChange[$TblSetting[$key]['name']]).
 * "&field_type=".$TblSetting[$key]['type']. - тип пол€ поле текстовое изсправчника или св€зь по идентификатору
 * "&tbl_spr=".$TblSetting[$key]['directory_table']. - таблица справочника
 * "&irn=".$TblSetting[$key]['name'], - им€ пол€ формы дл€ возврата из окошка (если св€зь по id, то указываем идентификатор)
 *
 */

	//ссылка на откртое окно
	...
	}else{$field_type = "";} // - тип пол€


	if($_GET["tbl_spr"] != ""){
		$tbl_spr_cfg = tblGetConfig($_GET["tbl_spr"],$arrSetting);
		if(!$tbl_spr_cfg["table"]['is_directory']){
			echo Message("“аблица: ".$_GET["tbl_spr"].", не €вл€етс€ справочником!","error");
			exit;
		}
		$path_lnk.= "&tbl_spr=".$_GET['tbl_spr']."&irn=".$_GET['irn'];
	}
	else{
		echo Message("tbl_spr - Ќе определено","error");
		exit;
	}
	...
	if($field_type == "directory_id"){
		if($tmp_code = spr_GetSelectedCodeById($sql, $tbl_spr_cfg, $selected_val)){
	...
	if($tbl_spr_cfg["table"]["directory_NoAddInWindow"] != 0){}
		...
		//if(usr_Access("edit")){
			$return_var.= spr_windowSetTpl("<a href='".$now_link."&add=yes' id='id_add_row' title='ƒобавить запись'><img src='".$arrSetting['Path']['ico']."/add.gif'/></a>", "ico");
		//}
	}
	...
	$return_var.= spr_windowSetTpl("<a href='javascript:void(0);' OnClick=\"$.nmTop().close();\">«акрыть</a>", "min_block", "right");
	$return_var.= spr_windowSetTpl("<a href='".$path_lnk."' id='id_back'>¬ начало</a>", "min_block", "right");
	...
	if(isset($_POST["dfghetserhdfjgj"]) && $_POST["dfghetserhdfjgj"] != ""){
		...
		if($sql->sql_insert($tbl_spr_cfg["table"]['name'],$ArrFV['ListField'],$ArrFV['ListValue'])){$ut->utLog(__FILE__ . " - запись сохранена");}else{$ut->utLog(__FILE__ . " - ошибка сохранени€ записи");}

		if($field_type == "directory_id"){
			$show_window_content.= Message("ƒобавлена запись: 
			<a href='#' OnClick=\"
					jsAddField(
					'".$_GET['irn']."',
		...
		*/
		...
		foreach($tbl_spr_cfg["sortfieldform"] as $key=>$val){
			if($tbl_spr_cfg[$key]['editable'] == 1 && $tbl_spr_cfg[$key]['type'] != 'support'){
				...
				// если запись нова€, то разрешаем редактировать пол€ только дл€ чтени€
				...
				// дл€ перехода не предыдущий уровень
		...
		$return_var.= frmInput(array("type"=>"submit","name"=>"SaveEditRowWindow","id"=>"SaveEditRowWindow","value"=>"—охранить","title"=>"",));
		$return_var.= "</form>";
		...
		if($field_type == "directory_id"){
			$spr_list_arr = spr_list_dir_id($sql, $arrSetting, $tbl_spr_cfg, $path_lnk, $var_where);
		}
		else{
			if(!$MultiSelect){
